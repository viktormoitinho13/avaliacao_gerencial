...
O projeto, além de ser uma aplicação web possuí conexão com o framework Procfit. Essa conexão se dá devido aS consultas e formulários criados para a educação corporativa. 
	#### 1 - Formulário de questões será utilizado para que a educação corporativa possa incluir, editar ou excluir questões das avaliações gerenciais 
